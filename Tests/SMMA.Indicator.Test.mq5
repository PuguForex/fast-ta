...
#property indicator_label1  "Streaming_SMMA"
#property indicator_type1   DRAW_LINE
#property indicator_color1  clrDodgerBlue
#property indicator_style1  STYLE_SOLID
#property indicator_width1  2

#include "..\\Indicators\\SMMA.mqh"

// Input parameter to control the window size dynamically
input int InpPeriod = 20; // SMMA Period

// Instantiate the encapsulated SMMA object globally
SMMA g_smma;
...
int OnInit()
{
// 1. Configure the SMMA instance
   g_smma.SetParameters(InpPeriod);

// 2. Initialize internal components via overridden lifecycle hook
   if(!g_smma.Init())
   {
      Print("FAIL: SMMA Component Initialization failed.");
      return(INIT_FAILED);
   }
...
// 3. Bind raw array to the MT5 indicator core execution engine
   ...
   PrintFormat("PASS: SMMA Pipeline Initialized with Period: %d", InpPeriod);
...
// Handle full recalculation environments cleanly
   if(limit == 0)
   {
      g_smma.Reset(); // Falls back to clearing components
   }
...
// --- MAIN PIPELINE LOOP ---
   for(int i = limit; i < rates_total && !_StopFlag; i++)
   {
      ...
      g_plotBuffer[i] = g_smma.GetValue(0, SMMA::MAIN);

      if(i == rates_total - 2)
      {
         for(int j = 0; j < InpPeriod && i - j >= 0; j++)
         {
            PrintFormat("g_plotBuffer[%i] = %f, g_smma.GetValue(%i, SMMA::MAIN) = %f, equal? = %s",
                        i - j, g_plotBuffer[i - j], j, g_smma.GetValue(j, SMMA::MAIN), g_plotBuffer[i - j] == g_smma.GetValue(j, SMMA::MAIN) ? "True" : "False");
         }
      }
      else if(i == rates_total - 1)
      {
         int j = 0;
         Comment(StringFormat("g_plotBuffer[%i] = %f, g_smma.GetValue(%i, SMMA::MAIN) = %f, equal? = %s",
                              i - j, g_plotBuffer[i - j], j, g_smma.GetValue(j, SMMA::MAIN), g_plotBuffer[i - j] == g_smma.GetValue(j, SMMA::MAIN) ? "True" : "False"));
      }
   }
