...
#property indicator_label1  "Streaming_EMA"
...
// Instantiate the encapsulated EMA object globally
EMA g_ema;
...
int OnInit()
{
// 1. Configure the EMA instance
   g_ema.SetParameters(InpPeriod);

// 2. Initialize internal components via overridden lifecycle hook
   if(!g_ema.Init())
   {
      Print("FAIL: EMA Component Initialization failed.");
      return(INIT_FAILED);
   }
...
// 3. Bind raw array to the MT5 indicator core execution engine
   ...
   PrintFormat("PASS: EMA Pipeline Initialized with Period: %d", InpPeriod);
...
// Handle full recalculation environments cleanly
   if(limit == 0)
   {
      g_ema.Reset(); // Falls back to clearing components
   }
...
// --- MAIN PIPELINE LOOP ---
   for(int i = limit; i < rates_total && !_StopFlag; i++)
   {
      ...
      g_plotBuffer[i] = g_ema.GetValue(0, EMA::MAIN);

      if(i == rates_total - 2)
      {
         for(int j = 0; j < InpPeriod && i - j >= 0; j++)
         {
            PrintFormat("g_plotBuffer[%i] = %f, g_ema.GetValue(%i, EMA::MAIN) = %f, equal? = %s",
                        i - j, g_plotBuffer[i - j], j, g_ema.GetValue(j, EMA::MAIN), g_plotBuffer[i - j] == g_ema.GetValue(j, EMA::MAIN) ? "True" : "False");
         }
      }
      else if(i == rates_total - 1)
      {
         int j = 0;
         Comment(StringFormat("g_plotBuffer[%i] = %f, g_ema.GetValue(%i, EMA::MAIN) = %f, equal? = %s",
                              i - j, g_plotBuffer[i - j], j, g_ema.GetValue(j, EMA::MAIN), g_plotBuffer[i - j] == g_ema.GetValue(j, EMA::MAIN) ? "True" : "False"));
      }
   }
