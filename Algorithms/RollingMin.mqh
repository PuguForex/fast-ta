/**
 * @file RollingMin - Stateful O(1) amortized rolling minimum algorithm using delayed live-value insertion and monotonic deque storage.
 * @deps MonotonicDeque.mqh
 * @state Stateful (Maintains committed historical minimum candidates and a separately tracked live value)
 * @io   Appended or replaced double values -> Updates rolling minimum state | Request value -> Returns current rolling minimum
 * @note AI-assisted code. Independently review and test in a demo environment before live production trading.
 */

#ifndef __ROLLING_MIN_MQH__
#define __ROLLING_MIN_MQH__
...
//| PURPOSE                                                          |
//| -------                                                          |
//| Maintains a rolling minimum using O(1) amortized operations.      |
//|                                                                  |
//| RESPONSIBILITIES                                                 |
//| ----------------                                                 |
//| • Maintain committed historical minimum candidates              |
...
public:
   ...
   double Value() const
   {
      ...
      return(MathMin(frontValue, m_liveValue));
   }
};

#endif // __ROLLING_MIN_MQH__
//+------------------------------------------------------------------+
